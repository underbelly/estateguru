var AppModule = (() => {
  ...
  var VideoBox = class {
    ...
    showVideo() {
      this.appendVideo();
      gsapWithCSS.to(this.thumbnail, {
        autoAlpha: 0,
        duration: 0.5,
        ease: "power2.inOut"
      });
      gsapWithCSS.to(this.video, {
        autoAlpha: 1,
        duration: 0.5,
        ease: "power2.inOut"
      });
      gsapWithCSS.to(this.playButton, {
        autoAlpha: 0,
        duration: 0.5,
        ease: "power2.inOut"
      });
      if (this.variant === "base") {
        ...
        !this.isMobile && gsapWithCSS.to(this.textContent, {
          autoAlpha: 0,
          duration: 0.5,
          ease: "power2.inOut"
        });
        gsapWithCSS.set(this.thumbnail, {
          height: this.wrap.offsetHeight,
          width: this.wrap.offsetWidth
        });
        gsapWithCSS.set(this.wrap, {
          gridArea: "1 / 1 / 1 / 13",
          width: this.wrap.offsetWidth,
          height: this.wrap.offsetHeight
        });
    ...
    closeVideo() {
      ...
      if (this.variant === "base") {
        ...
        gsapWithCSS.to(this.textContent, {
          autoAlpha: 1,
          duration: 0.5,
          ease: "power2.inOut",
          delay: 0.5
  ...
  // js/classes/VideoCarousel.js
  var VideoCarousel = class {
    ...
    sizing() {
      ...
      gsapWithCSS.set(this.wrap, {
        height: this.isMobile ? this.convertToRem(this.isMobile ? this.minItemWidth * (this.maxVisibleItems - 1) + this.maxItemWidth + this.gap * (this.maxVisibleItems - 1) : this.carouselHeight) : "auto"
      });
      this.items.forEach((item, index) => {
        gsapWithCSS.set(item.text, {
    ...
    changeSlide(index) {
      ...
      const currentTL = gsapWithCSS.timeline();
      const currentSlide = this.items[normalIndex];
      gsapWithCSS.set(currentSlide.img, {
        autoAlpha: 1,
        scale: 1.4,
        clipPath: this.isMobile ? left ? "inset(100% 0% 0% 0%)" : "inset(0% 0% 100% 0%)" : left ? "inset(0% 100% 0% 0%)" : "inset(0% 0% 0% 100%)"
      });
  ...
  var VideoFAQ = class {
    ...
    openAnswer(index) {
      gsapWithCSS.to(this.answerBlock, {
        autoAlpha: 1,
        duration: 0.5,
        ease: "power2.inOut"
      });
  ...
  (function() {
    ...
    const filterGTM = (...args) => {
      ...
      if (!stack)
        return true;
      if (stack.includes("gtm.js") || stack.includes("VM") || stack.includes("normal?lang=auto") || stack.includes("challenges.cloudflare.com") || stack.includes("cdn-cgi") || stack.includes("turnstile")) {
        return;
      }
      if (message.includes("challenges.cloudflare.com") || message.includes("normal?lang=auto") || message.includes("cdn-cgi")) {
        return;
      }
      return true;
    };
